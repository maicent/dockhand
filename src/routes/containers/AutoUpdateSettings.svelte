		...
		<div class="flex items-start gap-2">
			...
			<div class="space-y-2 text-xs">
				{#if systemContainer === 'dockhand'}
					<p class="font-medium text-blue-600 dark:text-blue-400">{m.container_autoupdate_not_available()}</p>
					<p class="text-muted-foreground">
						{m.container_autoupdate_dockhand_hint()}
					</p>
				...
				{:else}
					<p class="font-medium text-blue-600 dark:text-blue-400">{m.container_autoupdate_not_available()}</p>
					<p class="text-muted-foreground">
						{m.container_autoupdate_hawser_hint()}
					</p>
					<a
					...
					>
						...
						{m.container_autoupdate_view_instructions()}
					</a>
				{/if}
			</div>
		</div>
...
{:else}
	<div class="space-y-3">
		<div class="flex items-center gap-3">
			<Label class="text-xs font-normal">{m.container_autoupdate_enable_label()}</Label>
			<TogglePill
		...
		{#if enabled}
			...
			{#if envHasScanning}
				<div class="space-y-1.5">
					<Label class="text-xs font-medium">{m.container_autoupdate_vuln_criteria_label()}</Label>
					<VulnerabilityCriteriaSelector
						bind:value={vulnerabilityCriteria}
						onchange={(v) => oncriteriachange?.(v)}
					/>
					<p class="text-xs text-muted-foreground">
						{m.container_autoupdate_block_vuln_desc()}
					</p>
				</div>
			{/if}
		{/if}
